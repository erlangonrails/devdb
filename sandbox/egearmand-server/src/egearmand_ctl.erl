-module(egearmand_ctl).
% remote control of the server

-export([get_config_file/0, get_log_file/0]).
-export([start/0, process/1]).

-define(DEFAULT_CONFIG_FILE, "egearmand.cfg").
-define(DEFAULT_LOG_FILE, "egearmand.log").
...
process(["status"]) ->
    ...
    case lists:keysearch(egearmand, 1, application:which_applications()) of
        false ->
            io:format("egearmand is not running~n", []),
            ?STATUS_ERROR;
        {value,_Version} ->
            io:format("egearmand is running~n", []),
            ?STATUS_SUCCESS
    end;
