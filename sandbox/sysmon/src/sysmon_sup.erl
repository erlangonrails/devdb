...
%% @doc Supervisor for the sysmon application.
...
init([]) ->
    Ip = sysmon:get_config(ip, "127.0.0.1"),
    Port = sysmon:get_config(port, 8000),
    DocRoot = sysmon_deps:local_path(["priv", "www"]),
    WebConfig = [
         {ip, Ip},
         {port, Port},
         {docroot, DocRoot}],
    ...
    Web = {sysmon_web,
           {sysmon_web, start, [WebConfig]},
           permanent, 5000, worker, dynamic},

    %% get the base dir
    BaseDir = sysmon_deps:get_base_dir(),
