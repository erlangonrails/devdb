...
-define(DEFAULT_CONFIG_FILE, "skel.cfg").
-define(DEFAULT_LOG_FILE, "skel.log").
...
process(["status"]) ->
    ...
    case lists:keysearch(skel, 1, application:which_applications()) of
        false ->
            io:format("skel is not running~n", []),
            ?STATUS_ERROR;
        {value,_Version} ->
            io:format("skel is running~n", []),
            ?STATUS_SUCCESS
    end;
