...
-record(state, {
	  ...
	  be_list,				% Back-end list -> #be{} list
	  acceptor,				% Pid of listener proc
	  start_time,				% Proxy start timestamp -> now()
	  to_timer,				% Timeout timer ref -> the time ref of check_waiter_timeouts
	  wait_list				% List of waiting clients -> queue
	 }).
...
-record(be, {
	  ...
	  status,				% up|down
	  maxconn,				% Maximum connections -> 这个host最大的并发连接数
	  pendconn = 0,				% Pending connections -> 当前的pending连接数量
	  actconn = 0,				% Active connections  -> 当前的active连接数量
	  lasterr,				% Last error (term)   -> 最近发生的错误
	  lasterrtime,				% Timestamp of last error -> 最近发生错误的时间
	  act_count = 0,			% Times backend has been active -> 从启动开始, 累计接收了多少的TCP连接
	  act_time = 0,				% Cumulative activity time -> 从启动开始, 累计proxy了多少时间?
	  pidlist = []				% Pending & active pid list -> {pending, Pid} | {active, Pid, now()}
	 }).
...
%% Note:
%% get_be/1, remote_ok/1, remote_error/2都是在proxy进程中调用的.
%% 每个proxy对应了一对TCP连接: 
%% Client <-> Proxy <-> Back-end Host
...
add_be(BalancerPid, NewBE, AfterHost) when is_record(NewBE, be) ->
...
handle_call({Pid, get_be}, From, State) ->
    ...
    case Reply of
	?MUST_WAIT ->
	    {noreply, NewState}; %% 将调用gen_server:reply/2显示的返回结果 :)
	Reply ->
...
%% get the back-end host list from the cfg file.
get_be_list() ->
    case balance:get_config('initial_be_list', undefined) of
	undefined ->
	    ?WARNING_MSG("get_be_list: warning: cannot file 'initial_be_list' in application environment\n", []),
	    [];
	BeList ->
	    BeList
    end.
...
do_add_be(State, NewBE, AfterHost) when is_record(NewBE, be) ->
...
format_be_list([B|Bs], Acc) ->
    LastErrTime = if
		      is_tuple(B#be.lasterrtime) -> B#be.lasterrtime;
		      true -> {0,0,0}
		  end,
    format_be_list(Bs, [[
			 "<tr> ",
