...
%% 
%% 测试:
%% http://localhost:8004/timer  - 每次刷新这个页面, Count会增加一次
...
%% Internal API
route_req_get(Req, "timer", _DocRoot) ->
    CookieC = case Req:get_cookie_value("count_key") of
                  undefined ->
		      "1";
		  CountKey ->
		      CountKey1 = list_to_integer(CountKey) + 1,
		      integer_to_list(CountKey1)
              end,
    CookieKey2 = mochiweb_cookies:cookie("count_key", CookieC, []),
    Req:respond({200, [{"Content-Type","text/html"}, CookieKey2], 
		 io_lib:format("<h1>count_key: ~s~n</h1>", [CookieC])});
